...
type
  TForm_filiere = class(TForm)
    ...
    procedure libellerKeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  begin
    ...
    begin
      ...
      ShowMessage('Filière enregistrer  ave success!') ;
  end
  else
...
procedure TForm_filiere.codeKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TForm_filiere.FormActivate(Sender: TObject);
begin
  code.Clear;
  libeller.Clear;
end;

procedure TForm_filiere.libellerKeyPress(Sender: TObject; var Key: Char);
begin
  if not (key  in ['a'..'z','A'..'Z','0'..'9',#8] ) then
    begin
         key :=#0;
